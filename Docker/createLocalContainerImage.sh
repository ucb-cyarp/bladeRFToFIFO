...
cd $scriptSrc
echo "docker build -t bladerf-to-fifo-remote-cpp-env-local:${timestamp} -f Dockerfile ."
docker build --progress=plain -t "bladerf-to-fifo-remote-cpp-env-local:${timestamp}" -f Dockerfile . 2>&1 | tee dockerBuild.log
