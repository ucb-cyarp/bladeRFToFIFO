...
echo "docker run -d --cap-add sys_ptrace -p127.0.0.1:2223:22 -v $projectDir:/project: --name bladerf-to-fifo-clion_remote_env bladerf-to-fifo-remote-cpp-env-local:latest"
docker run -d --cap-add sys_ptrace -p127.0.0.1:2223:22 -v "$projectDir":/project: --name bladerf-to-fifo-clion_remote_env bladerf-to-fifo-remote-cpp-env-local:latest
